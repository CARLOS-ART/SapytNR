...
var $parent = $container.parent();
  ...
  var config = {
    ...
    minRows: 1,
    rowHeaders: true,
    colWidths: [70,100, 320, 90, 90, 100, 100,150],
    minCols: 8,
    minSpareRows: 0,
    autoWrapRow: true,
    height:510,
    colHeaders: ['SAP','CLAVE', 'PRODUCTO', 'P. COMPRA', 'P. VENTA', 'GR CON TARA','GR SIN TARA','GR FISICO CON TARA'],
    contextMenu: false,
  ...
  $.ajax({
    url:'/Auditorias/Formato/Lab/Terminar',
    type:'post',
...
function Busqueda_Productos(e,Filtro){
  if(esEnter(e)){
    ...
    var Json = JSON.stringify($container.handsontable('getData'));
    $.ajax({
      url:'/Auditorias/Formato/Lab/FiltrarProductos',
      type:'post',
...
function CargarDatos_Lab(){
  var _token = $('input[name="_token"]').val();
  $.ajax({
    url:'/Auditorias/Formato/Lab/CargarDatos',
    type:'post',
    data:{_token:_token},
    beforeSend:function(){
     H5_loading.show();
