...
function CerrarBitacora1(){
  var Bit = $("#Bitacora").val();
        var _token = $('input[name="_token"]').val();
        $.ajax({
          url:'/Bitacoras/Tablero/CerrarOT',
          type:'post',
...
function MostrarB(Bit) {
  var _token = $('input[name="_token"]').val();
  //var coments = document.getElementById('comentarios').value;
  console.log('coments');
  $.ajax({
    url:'/Bitacoras/MostrarBit',
    type:'post',
    data:{_token:_token,Bit:Bit},
    success:function(data){
     //swal(data.Titulo,data.Mensaje,data.TMensaje);
     document.getElementById('Bitacora').value=data.Bitacora;
...
function CancelarBitacora(){
  var Bita = $("#Bitacora1").val();

        var _token = $('input[name="_token"]').val();

    console.log(Bita);
        $.ajax({
          url:'/Bitacoras/Tablero/CancelarOT',
          type:'post',
          data:{_token:_token,Bita:Bita},
          success:function(data){
           CircleNotification(data.Titulo,data.Mensaje,data.TMensaje,"top-left");
          }
  });
...
function MostrarBitCancelar(Bit) {
  var _token = $('input[name="_token"]').val();
  //var coments = document.getElementById('comentarios').value;
  console.log('coments');
  $.ajax({
    url:'/Bitacoras/MostrarBitCancelar',
    type:'post',
    data:{_token:_token,Bit:Bit},
    success:function(data){
     //swal(data.Titulo,data.Mensaje,data.TMensaje);
     document.getElementById('Bitacora1').value=data.Bitacora;
